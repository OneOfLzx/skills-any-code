...
- **输出形态**：在项目目录下（默认 `./.skill-any-code-result`）生成：
...
- **命令名**：`skill-any-code`（别名：`sac`）
- **全局选项**（`src/cli.ts`）：
  - `-c, --config <path>`：配置文件路径（默认 `~/.config/skill-any-code/config.yaml`）
...
- **默认路径**：`~/.config/skill-any-code/config.yaml`
...
- `SKILL_ANY_CODE_LOG_LEVEL` → `global.log_level`
- `SKILL_ANY_CODE_OUTPUT_FORMAT` → `global.output_format`
- `SKILL_ANY_CODE_AUTO_CONFIRM` → `global.auto_confirm`
- `SKILL_ANY_CODE_OUTPUT_DIR` → `global.output_dir`
- `SKILL_ANY_CODE_LLM_BASE_URL` → `llm.base_url`
- `SKILL_ANY_CODE_LLM_API_KEY` → `llm.api_key`
- `SKILL_ANY_CODE_LLM_MODEL` → `llm.model`
- `SKILL_ANY_CODE_LLM_CONTEXT_WINDOW_SIZE` → `llm.context_window_size`
- `SKILL_ANY_CODE_LLM_CACHE_ENABLED` → `llm.cache_enabled`
- `SKILL_ANY_CODE_LLM_CACHE_DIR` → `llm.cache_dir`
...
sequenceDiagram
  ...
  User->>CLI: skill-any-code --path P --mode auto ...
...
export class BlacklistService implements IBlacklistService {
  ...
  async load(globalBlacklist: string[], projectRoot: string): Promise<void> {
    ...
    // 第二层：项目级黑名单（.skill-any-code-ignore）
...
2. **三层加载顺序**：全局 → `.skill-any-code-ignore` → `.gitignore`，后出现的否定模式可覆盖前面的规则。
...
| Provider 标识 | 部署目录 | 备注 |
|--------------|---------|------|
| `opencode` | `{项目根目录}/.agents/skills/skill-any-code/` | 共享 `.agents/skills/` |
| `cursor` | `{项目根目录}/.agents/skills/skill-any-code/` | 共享 `.agents/skills/` |
| `codex` | `{项目根目录}/.agents/skills/skill-any-code/` | 共享 `.agents/skills/` |
| `claude` | `{项目根目录}/.claude/skills/skill-any-code/` | Claude Code 专用 |
...
```json
{
  "indexFilePath": "C:/Software/my-project/.skill-any-code-result/analysis-index.json"
}
```
...
移除：
- `SKILL_ANY_CODE_QUERY_DEFAULT_TYPE` → 废弃

新增：
- `SKILL_ANY_CODE_SKILLS_DEFAULT_PROVIDERS` → `skills.default_providers`（逗号分隔字符串，解析为数组）
...
| 仅全局配置 + `.gitignore` 两层 | 全局配置 + `.skill-any-code-ignore` + `.gitignore` 三层 |
...
- 二进制文件（由文本/二进制检测判定）；
- 被三层黑名单过滤的文件/目录（全局 blacklist + `.skill-any-code-ignore` + `.gitignore`）；
- 解析结果自身（`.skill-any-code-result/` 目录）；
...
在 V2.3 及更早版本中，`skill-any-code` 在真正进入解析流程前，会在 CLI 层发起两类交互式确认：
...
- **统计周期**：一次完整的解析命令执行（一次 `skill-any-code` 调用），无论是全量还是增量；
...
- **可感知的初始化步骤**：通过显式 `skill-any-code init` 创建 / 覆盖配置文件，而不是在首次执行其他命令时隐式生成。
...
- 配置文件路径解析规则保持与现有设计一致：
  ...
  - 否则，使用默认路径：`~/.config/skill-any-code/config.yaml`（跨平台通过 `os.homedir()` + `path.join` 计算）。
...
- `skill-any-code init [-c, --config <path>]`
  ...
  async load(configPath: string): Promise<CliConfig> {
    if (!(await fs.pathExists(configPath))) {
      ...
      throw new AppError(
        ...
        `配置文件未初始化，请先执行 "skill-any-code init" 创建配置：${configPath}`,
      )
    }
...
- 已经存在配置文件的用户：
  ...
  - 可通过直接查看配置文件检查当前 LLM 与缓存相关配置。
- 新用户或在新机器上的首次使用：
  - 必须先执行 `skill-any-code init` 创建配置文件；
