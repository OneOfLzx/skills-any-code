...
// 在输出靠近尾部的位置尝试解析一个「进度 -> 当前对象块 -> Tokens」的顺序片段
function findFinalBlockTriple(output: string) {
  ...
  // 从最后一次 Token 行开始向前尝试，找到任意一组满足约束的三元块即可
  for (let t = tokenIndices.length - 1; t >= 0; t--) {
    ...
    if (currentHeaderIndex === -1) continue;
    ...
    for (let i = currentHeaderIndex - 1; i >= 0; i--) {
      ...
      if (currentHeaderIndex - i > 500) break;
    }
...
describe('CLI 输出结构稳健性：进度/当前对象/Tokens 行 (问题3 回归)', () => {
  ...
  itWithDist(
    ...
    async () => {
      ...
      expect(tokenSnapshots.length).toBeLessThanOrEqual(
        totalObjects * TOKENS_MULTIPLIER,
      );
    },
    300000,
  );
  ...
  itWithDist(
    '输出顺序/格式契约：在最终阶段至少出现一次「解析进度 -> 当前对象块 -> Tokens」三段式结构',
    async () => {
      const { code, output } = await runMediumProjectFullAnalyze(3);
      expect(code).toBe(0);
      ...
      if (!triple) {
        // 在本用例参数下，当前对象块与 Tokens 行都是预期输出；缺失则视为回归
        expect(groups.length).toBeGreaterThan(0);
        expect(tokenSnapshots.length).toBeGreaterThan(0);
        throw new Error(
          '输出中同时存在当前对象块与 Tokens 行，但未能找到期望的「解析进度 -> 当前对象块 -> Tokens」三段式结构',
        );
      }

      const { progressIndex, currentHeaderIndex, tokenIndex, pathLines } =
        triple!;
